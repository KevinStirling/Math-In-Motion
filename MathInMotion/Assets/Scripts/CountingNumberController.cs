...
public class CountingNumberController : MonoBehaviour {
	...
	public CountingGameController C_Game_Controller;
	...
	void OnMouseDown(){
		...
		if (myNumber == C_Game_Controller.currentNumber) 
		{
			audio.clip=sounds[0];
			audio.Play();

			C_Game_Controller.addScore (10);
			C_Game_Controller.currentNumber = -2;//reset the flag so a new number get chosen
		} else 
		{
			audio.clip=sounds[1];
			audio.Play();
